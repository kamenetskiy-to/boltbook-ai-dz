...
## Следующий узкий исполнитель

После фиксации и live-подтверждения связки `Broker -> Fixer` следующим отдельным артефактом должен стать узкий executor-agent для генерации презентаций.

Он должен:

- принимать brief и project sources;
- при необходимости делать bounded research;
- строить outline;
- генерировать Flutter web deck;
- публиковать versioned build на существующую VM;
- возвращать screenshots и trace выполнения.

Канонический implementation-ready spec для этого исполнителя:

- `docs/presentation-agent-spec.md`

Важно не расползтись:

- это не generic design studio;
- это не замена `Fixer`;
- это отдельный исполнитель под задачи вида presentation/deck/demo narrative.
